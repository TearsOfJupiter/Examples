...
public class TryTest
{
  ...
  private static final Consumer<Integer> KABLOOEY_CONSUMER = i -> {throw new RuntimeException(KABLOOEY);};
  private static final BiConsumer<Throwable, Integer> KABLOOEY_BICONSUMER = (e, i) -> {throw new RuntimeException(KABLOOEY);};
  private static final VariadicConsumer<Integer> KABLOOEY_VARCONSUMER = vals -> {throw new RuntimeException(KABLOOEY);};
  private static final Supplier<Throwable> KABLOOEY_SUPPLIER = () -> new RuntimeException(KABLOOEY);
  private static final Supplier<Integer> KABLOOEY_INTSUPPLIER = () -> {throw new RuntimeException(KABLOOEY);};
  private static final Supplier<Try<Integer>> KABLOOEY_TRYSUPPLIER = () -> {throw new RuntimeException(KABLOOEY);};
  private static final Function<Integer, Integer> KABLOOEY_FUNCTION = i -> {throw new RuntimeException(KABLOOEY);};
  private static final VariadicFunction<Integer, String> KABLOOEY_VARFUNCTION = (Integer... array) -> {throw new RuntimeException(KABLOOEY);};
  private static final Predicate<Integer> KABLOOEY_PREDICATE = i -> {throw new RuntimeException(KABLOOEY);};
  private static final Function<Integer, Try<Integer>> GOOD_FLATMAP = val -> Try.of(i -> i / 2, val);
  private static final Function<Integer, Try<Integer>> BAD_FLATMAP = val -> Try.of(i -> i / 0, val);
  ...
  {
    final int val = 1;
    final Try<Integer> ts = Try.of(i -> i / 1, val);
    assertTrue(ts.isSuccessful());
    assertFalse(ts.isFailure());
    assertTrue(ts.getFailure().isEmpty());
    assertEquals((int) ts.orElseThrow(), val);

    final Try<Integer> tf = Try.of(i -> Integer.parseInt(i) / 0, String.valueOf(val));
    assertFalse(tf.isSuccessful());
    assertTrue(tf.isFailure());
  ...
  public void test_of_Consumer()
  {
    ...
    final Try<Integer> tf = Try.of(KABLOOEY_CONSUMER, 1);
    assertFalse(tf.isSuccessful());
    assertTrue(tf.isFailure());
  ...
  /**
   * Demonstrates {@link Try#of(VariadicConsumer, Object[])}
   */
  @Test
  public void test_of_VariadicConsumer()
  {
    ...
    final Try<Integer[]> ts = Try.of(goodConsumer, 1, 2, 3);
    assertTrue(ts.isSuccessful());
    assertFalse(ts.isFailure());
    assertTrue(ts.getFailure().isEmpty());
    ...
    final Try<Integer[]> tf = Try.of(KABLOOEY_VARCONSUMER, 1, 2, 3);
    assertFalse(tf.isSuccessful());
    assertTrue(tf.isFailure());
    final Pair<Integer[], Throwable> pair = tf.<Integer[]>getFailure().orElseThrow();
  ...
  public void test_of_VariadicFunction()
  {
    ...
    final Try<String> tf = Try.of(KABLOOEY_VARFUNCTION, 1, 2, 3);
    assertFalse(tf.isSuccessful());
    assertTrue(tf.isFailure());
    final Pair<Integer[], Throwable> pair = tf.<Integer[]>getFailure().orElseThrow();
    final List<Integer> tfVals = Arrays.asList(pair.getA());
  ...
  public void test_ifSuccessful()
  {
    ...
    final Try<Integer> tf = Try.of(() -> 2 / 1).ifSuccessful(KABLOOEY_CONSUMER);
    assertTrue(tf.isFailure());
  ...
  public void test_ifSuccessfulOrElse_Consumer()
  {
    ...
    // Bad consumer
    Try.of(KABLOOEY_CONSUMER, 0).ifSuccessfulOrElse(ints::add, failedDivisors::add);
    ...
    final Try<Integer> tf = Try.of(KABLOOEY_CONSUMER, 10).ifSuccessfulOrElse(ints::add, KABLOOEY_CONSUMER);
    assertTrue(tf.isFailure());
  ...
  public void test_ifSuccessfulOrElse_BiConsumer()
  {
    ...
    // Good consumer
    Try.of(goodConsumer, 2).ifSuccessfulOrElse(ints::add, (e, i) -> failedDivisors.add(new Pair<>(e, i)));
    assertEquals(ints.size(), 1);
    assertTrue(ints.contains(2));
    ...
    // Bad consumer
    Try.of(KABLOOEY_CONSUMER, 0).ifSuccessfulOrElse(ints::add, (e, i) -> failedDivisors.add(new Pair<>(e, i)));
    assertEquals(ints.size(), 0);
    assertEquals(failedDivisors.size(), 1);
    ...
    final Try<Integer> tf = Try.of(KABLOOEY_CONSUMER, 10).ifSuccessfulOrElse(ints::add, KABLOOEY_BICONSUMER);
    assertTrue(tf.isFailure());
    final Pair<Integer, Throwable> failure = tf.<Integer>getFailure().orElseThrow();
    assertEquals(failure.getA(), 10);
    assertEquals(failure.getB().getMessage(), KABLOOEY);
  ...
  public void test_ifSuccessfulOrElseDo()
  {
    ...
    // Null runnable
    final Try<Integer> tf = Try.of(i -> i / 0, 4)
  ...
  public void test_ifFailure_BiConsumer()
  {
    ...
    final Try<Integer> tf2 = Try.of(KABLOOEY_CONSUMER, 4)
        .ifFailure((BiConsumer<? super Throwable, ? super Integer>) null);
  ...
  {
    assertTrue(Try.of(i -> i / 3, 3)
        .getFailure().isEmpty());

    // Demonstrates without explicit generics
    final Try<Integer> tf = Try.of(i -> i / 0, 3);
  ...
  public void test_filter()
  {
    ...
    final Try<Integer> ts = Try.of(s -> Integer.parseInt(s) / 3, String.valueOf(9))
        .filter(val -> val % 3 == 0);
    assertTrue(ts.isSuccessful());
    assertEquals((int) ts.orElseThrow(), 3);

    // Good function, good predicate, value fails predicate
    final Try<Integer> tf1 = Try.of(s -> Integer.parseInt(s) / 3, String.valueOf(9))
        .filter(val -> val % 2 == 0);
    ...
    final Try<Integer> tf2 = Try.of(s -> Integer.parseInt(s) / 3, String.valueOf(9))
    ...
    // Bad function
    final Try<Integer> tf3 = Try.of(i -> Integer.parseInt(i) / 0, String.valueOf(3))
        .filter(val -> val % 2 == 0);
    ...
    // Null predicate
    final Try<Integer> tf4 = Try.of(i -> i / 3, 9)
        .filter(null);
  ...
  public void test_map()
  {
    ...
    final int result = Try.of(i -> i / 3, 9)
        .map(i -> i % 3)
        .orElseThrow();
    assertEquals(result, 0);

    // Good function, bad mapper
    final Try<Integer> tf = Try.of(i -> i / 3, 9)
    ...
    final Try<Integer> tf2 = Try.of(i -> i / 0, 4)
        .map(i -> i % 2);
    assertTrue(tf2.isFailure());
    final Pair<Object, Throwable> failure2 = tf2.getFailure().orElseThrow();
    assertEquals(failure2.getA(), 4);
    ...
    // Null mapper
    final Try<Integer> tf3 = Try.of(i -> i / 2, 4)
        .map(null);
    assertTrue(tf3.isFailure());
    final Pair<Object, Throwable> failure3 = tf3.getFailure().orElseThrow();
  ...
  {
    final Try<Integer> t = Try.of(i -> i / 4, 8);
    ...
    // Bad function
    final Try<Integer> tf3 = Try.of(i -> i / 0, 4)
        .flatMap(GOOD_FLATMAP);
    assertTrue(tf3.isFailure());
    final Pair<Object, Throwable> failure3 = tf3.getFailure().orElseThrow();
  ...
  {
    final Integer val = Try.of(i -> i / 2, 4).stream().findFirst().orElseThrow();
    assertEquals((int) val, 2);

    assertTrue(Try.of(i -> i / 0, 4).stream().findFirst().isEmpty());
  }
  ...
  {
    assertEquals((int) Try.of(i -> i / 4, 8)
        .orElse(5), 2);

    assertEquals((int) Try.of(i -> i / 0, 2)
        .orElse(1), 1);
  }
  ...
  {
    assertEquals((int) Try.of(i -> i / 2, 4)
        .orElseGet(() -> 5), 2);

    assertEquals((int) Try.of(i -> i / 0, 4)
        .orElseGet(() -> 5), 5);
